...
Script para análise de logs de acesso, com foco em identificar indícios de ataques web, desenvolvido na **Aula 3** da **RedScanAcademy Turma-12**.
...
- **`-a` Detectar XSS (Cross-Site Scripting)**  
  ...
  Exemplo:
  ```bash
  ./redscan3-1.sh -a <ARQUIVO_DE_LOG>
  ```
...
- **`-b` Detectar SQL Injection**  
  ...
  Exemplo:
  ```bash
  ./redscan3-1.sh -b <ARQUIVO_DE_LOG>
  ```
...
- **`-c` Detectar Directory Traversal**  
  ...
  Exemplo:
  ```bash
  ./redscan3-1.sh -c <ARQUIVO_DE_LOG>
  ```
...
- **`-d` Detectar scanners (User-Agent suspeito)**  
  ...
  Exemplo:
  ```bash
  ./redscan3-1.sh -d <ARQUIVO_DE_LOG>
  ```
...
- **`-e` Acesso a arquivos sensíveis**  
  ...
  Exemplo:
  ```bash
  ./redscan3-1.sh -e <ARQUIVO_DE_LOG>
  ```
...
- **`-f` Força bruta em arquivos/pastas**  
  ...
  Exemplo:
  ```bash
  ./redscan3-1.sh -f <ARQUIVO_DE_LOG>
  ```
...
- **`-i` Listar IPs e quantidade de requisições**  
  ...
  Exemplo:
  ```bash
  ./redscan3-1.sh -i <ARQUIVO_DE_LOG>
  ```
...
- **`-j` Acesso a arquivos sensíveis específicos**  
  ...
  Exemplo:
  ```bash
  ./redscan3-1.sh -j <ARQUIVO_DE_LOG>
  ```
